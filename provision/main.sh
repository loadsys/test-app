...
${0##*/}
    Primary provisioning script for a simple Loadsys demo app.

    (Starting to be) Designed to be used both by vagrant an on any
    Ubuntu 14.04 LTS (Precise) server. Provide the desired APP_ENV
...
sudo apt-get install -y git-core apache2
#sudo apt-get install -y php5 php5-curl php5-intl php5-mcrypt php5-memcached php5-mysql
...
sudo cp "${PROVISION_DIR}/010-simple.conf" /etc/apache2/sites-available/

sudo a2ensite 010-simple
